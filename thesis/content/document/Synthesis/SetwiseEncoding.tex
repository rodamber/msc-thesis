...
\[
  ...
  \quad \wedge
  \bigwedge_{f \in F'}
  \bigwedge_{p \in P_f} (1 \leq l_p \leq l_{r_f})
\]
...
\begin{enumerate*}[(1)]
\item that the value of $r_1$ is effectively equal to \lstinline{Index(name, c0, c1)};
...
program. For instance, the return value of \lstinline{Concat} be either the
output of the program, or one of the actual parameters of the same type of
