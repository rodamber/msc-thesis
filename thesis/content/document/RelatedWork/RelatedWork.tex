\chapter{Related Work}
\label{chapter:relatedWork}

% Based on \cite{Gulwani2017} plus work on the field since then.
% Explain fundamentals and then give an overview of the developments in the
% field since then.

% FIXME: \fixme{Few references to works from 2018 onwards.}{}

% Program synthesis has been used typically to infer programs in declarative or
% functional languages.

This chapter discusses the state of the art of the field of program synthesis.
% Section~\ref{sec:challenges} presents the inherent challenges of the problem,
% such as resolving ambiguity or the size of the search space;
section~\ref{sec:specifications} \pdfmarkupcomment[color=yellow]{presents the
various forms that specifications can take}{Huh, ugly.};
% section~\ref{sec:applications} presents applications of the problem;
section~\ref{sec:search-techniques} presents techniques to prune the search
space;
section~\ref{sec:pbe} refers to work done specifically in the area of \gls{pbe}.

\citeauthor{Gulwani2017}~\cite{Gulwani2017} presented an overview of the field
in as of 2017.

\subimport{Specifications/}{Specifications}
\subimport{SearchTechniques/}{SearchTechniques}
\input{ProgrammingByExamples}

% Overview chapter 3:
% - DSL design
% - Optimization
% Also:
% - Synthesis of programs that transform data
